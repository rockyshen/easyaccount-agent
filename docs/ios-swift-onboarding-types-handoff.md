...
4. 分类管理页：标题可用「我的分类」；顶部 Tab **只展示** `GET /api/actions` 返回的 3 项（收入/支出/内部转账）；**不要**再加「这些分类只属于你…」提示条，列表区域边距与其他子页一致
5. `actions` / `types` 本地缓存（见主 handoff §4「本地存储优化」），避免每次进页都打满网
