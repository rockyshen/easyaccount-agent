...
class TypeServiceTest {
    ...
    void createChild_rejectsNonRootParent() {
        ...
        when(typeDao.findById(34)).thenReturn(grandparentChild);
    ...
    void delete_softDisablesAndCascadesChildren() {
        ...
        root.setActionId(1);
        when(typeDao.findById(12)).thenReturn(root);

        Type child = new Type();
        child.setId(34);
        child.setParent(12);
        child.setDisable(false);
        when(typeDao.findByParent(12)).thenReturn(List.of(child));
    ...
    void delete_missingType_throwsNotFound() {
        when(typeDao.findById(anyInt())).thenReturn(null);
    ...
    void update_rejectsDemotingRootWithChildren() {
        ...
        when(typeDao.findById(12)).thenReturn(root);
        when(typeDao.findById(20)).thenReturn(otherRoot);
        when(typeDao.findByParent(12)).thenReturn(List.of(child));
